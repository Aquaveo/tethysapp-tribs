...
function initAndRender() {
  ...
  const { rerender } = render(MeshRender);
...
it("Calls graphics toggle visibility callback when visibilty button pressed", async () => {
  const { user, mesh, hideObject, revealObject } = initAndRender();
  const visibilityButton = screen.getAllByRole("button", { name: /Hidden/ })[0];
  await user.click(visibilityButton);
  expect(revealObject).toHaveBeenCalledWith(mesh.id);

  await user.click(visibilityButton);
  expect(hideObject).toHaveBeenCalledWith(mesh.id);
});

it("Calls graphics delete and delete callback when delete button pressed", async () => {
  const { user, deleteCallback, rerender, MeshRender } = initAndRender();
  const optionsButton = screen.getByRole("button", { name: /Options/ });
  await user.click(optionsButton);
  const deleteButton = await screen.findByRole("button", { name: /Delete/ });
  await user.click(deleteButton);

  await rerender(MeshRender);
...
it("Opens and then closes the Confirm Delete modal", async () => {
  const { user, rerender, MeshRender } = initAndRender();
  const optionsButton = screen.getByRole("button", { name: /Options/ });
  await user.click(optionsButton);
  const deleteButton = await screen.findByRole("button", { name: /Delete/ });
  await user.click(deleteButton);
