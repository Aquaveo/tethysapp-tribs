...
it("Shows all of the errors if all fields are empty after submitting", async () => {
  ...
  const invisibleDatasetTypeError = screen.queryByText("Please provide a type for the Dataset.");
  ...
  const nameError = screen.getByText("Please provide a name for the Dataset.");
  const datasetTypeError = screen.getByText("Please provide a type for the Dataset.");
...
it("Calls submit and close callbacks when submit button pressed.", async () => {
  ...
  render(
    <AddGenericDatasetModal onSubmit={submitCallback} onClose={closeCallback} show />
  );
