...
import AddMeshModal from "./AddMeshModal";

it("Creates a modal dialog", () => {
  render(<AddMeshModal show />);
...
it("Creates correct title for the modal dialog", () => {
  render(<AddMeshModal show />);
  const modal = screen.getByRole("dialog");
  const title = screen.getByRole("heading", { name: /Add Mesh/ });
...
it("Calls the close callback when close button is pressed", async () => {
  ...
  render(<AddMeshModal show onClose={mockCallback} />);
...
it("Shows all of the errors if all fields are empty after submitting", async () => {
  ...
  const invisibleNameError = screen.queryByText("Please provide a name for the Mesh.");
  const invisibleDatasetTypeError = screen.queryByText("Please provide a type for the Mesh.");
  ...
  const nameError = screen.getByText("Please provide a name for the Mesh.");
  const datasetTypeError = screen.getByText("Please provide a type for the Mesh.");
...
it("Calls submit and close callbacks when submit button pressed.", async () => {
  ...
  render(
    <AddMeshModal onSubmit={submitCallback} onClose={closeCallback} show />
  );
...
it("Resets all fields when Cancel Button is clicked", async () => {
  ...
  const { rerender } = render(<AddMeshModal show />);
  const nameInput = screen.getByLabelText(/Name/);
  await user.type(nameInput, "Baz");
  ...
  rerender(<AddMeshModal show={false} />);
