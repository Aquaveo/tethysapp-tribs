...
it("Shows all of the errors if all fields are empty after submitting", async () => {
  ...
  const nameError = screen.getByText("Please provide a name for the Raster.");
  const datasetTypeError = screen.getByText("Please provide a type for the Raster.");
...
it("Calls submit and close callbacks when submit button pressed.", async () => {
  ...
  render(
    <AddRasterModal onSubmit={submitCallback} onClose={closeCallback} show />
  );
