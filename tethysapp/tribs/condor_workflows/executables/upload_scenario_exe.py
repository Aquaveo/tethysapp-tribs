...
from tribs_adapter.resources import Scenario, Project
from tribs_adapter.services.tribs_spatial_manager import TribsSpatialManager


def run(
    resource_db_url, project_id, resource_id, input_archive, srid, geoserver_endpoint, geoserver_public_endpoint,
    geoserver_username, geoserver_password, status_key
):
    """
    ...
    try:
        ...
        scenario = resource_db_session.query(Scenario).get(resource_id)
        project = resource_db_session.query(Project).get(project_id)

        if scenario is None:
            raise RuntimeError(f'Could not find Resource of type "{Scenario.TYPE}" with id "{resource_id}".')
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            scenario.init(project=project, srid=srid, input_file=input_file, spatial_manager=gs_manager)

        scenario.set_status(status_key, Scenario.STATUS_SUCCESS)
        resource_db_session.commit()
        sys.stdout.write(f'\nSuccessfully processed {resource_id}\n')

    except Exception as e:
        if scenario:
            scenario.set_status(status_key, Scenario.STATUS_ERROR)
            resource_db_session.commit()
