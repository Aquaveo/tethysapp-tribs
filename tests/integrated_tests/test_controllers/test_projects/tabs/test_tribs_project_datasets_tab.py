...
def test_datasets_get_resources(
    ...
    mtpd_controller = TribsProjectDatasetsTab()
    ...
    summary_file_out = os.path.join(tmp_path, 'controllers', 'projects', 'tabs', 'datasets_get_resources', 'out.txt')
    os.makedirs(os.path.dirname(summary_file_out))
    summary_file_base = os.path.join(
        test_files, 'controllers', 'projects', 'tabs', 'datasets_get_resources', 'base.txt'
    )
...
def test_get_href_for_resource(
    ...
    mock_reverse = mocker.patch('tethysapp.tribs.controllers.projects.tabs.tribs_project_datasets_tab.reverse')
